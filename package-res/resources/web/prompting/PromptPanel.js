    ...
    function (_, Base, Logger, DojoNumber, i18n, Utils, GUIDHelper, WidgetBuilder, Dashboard, ParamDiff) {
      ...
      var _findErrorComponents = function(panelComponent) {
        ...
        if (panelComponent.components) {
          for (var i = 0; i < panelComponent.components.length; i++) {
            var item = panelComponent.components[i];
            if (item.promptType == "label" && item.type == "TextComponent" && item.isErrorIndicator) {
              result.push(item);
            }
          }
        }
      ...
      var PromptPanel = Base.extend({
        ...
        _changeErrors: function(param) {
          if (param.isErrorChanged) {
            ...
            if (errors && errors.length > 0) { // add new error labels
              var _isExistingErrComp = function(errors, errMsg) {
                var result = false;
                for (var errIndex in errors) {
                  if (errors[errIndex].label == errMsg) {
                    result = true;
                    break;
                  }
                }
              ...
              for (var errIndex in errors) {
                ...
                var isExist = _isExistingErrComp(existingErrors, error);
                if (!isExist) {
            ...
            } else { // remove error labels
              for (var i in existingErrors) {
                _removeChildComponent.call(this, panel, existingErrors[i]);
              }
              panel.cssClass = panel.cssClass ? panel.cssClass.replace(' error', '') : '';

              this.removeDashboardComponents(existingErrors);
            }
          }
        },
